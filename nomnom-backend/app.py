...
# --- MODIFIED: Profile Page Endpoint (Bug Fix) ---
...
def user_profile():
    try:
        ...
        # FIX: Handle cases where there are no meals or reviews to prevent errors
        ...
        if meals:
            ...
            if meal_restaurant_ids: # Ensure there are restaurants to query
        ...
        recent_meals = [{
            'meal_id': meal.id, 'restaurant_name': name,
            'date': meal.date.isoformat(), 'rating': rating
...
def register():
    ...
    try:
        ...
        db.session.rollback(); print("❌ Registration error:", e)
